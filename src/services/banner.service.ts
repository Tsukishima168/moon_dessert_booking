...
export async function listBanners(): Promise<BannerRow[]> {
  const rows = await findAllBanners()
  return rows.map(withBannerDefaults)
}

function withBannerDefaults(row: BannerRow): BannerRow {
  return {
    ...row,
    link_text: row.link_text ?? '立即查看',
  }
}
...
export async function createBanner(
  ...
  return withBannerDefaults(await insertBanner(payload))
}
...
export async function editBanner(
  ...
  const { link_text: _linkText, ...persistedUpdateData } = updateData
  return withBannerDefaults(await updateBanner(id, {
    ...persistedUpdateData,
