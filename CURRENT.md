...
- `npm run test:economy-v2` 已通過 migration apply、100 次冪等重送、偽造點數／事件、匿名 claim 綁定／過期／跨會員／政策限制、UTC 時區繞過、RLS／grants／search path、Gacha、Map proof、兌換／重複核銷、整筆與部分 reversal、100 路扣點及 20 路最後一件庫存競態。
- 2026-07-16 production linked dry-run 僅列出上述 6 個 migration，未包含 roles、seed 或未知版本；這是無寫入候選驗證，正式 push 前仍須再跑一次。
- Foundation commit `e08a188` 與 pending-claim hardening `947b1b5` 已推到 `codex/economy-v2-foundation-20260715`，Draft PR #17 保持未簽收狀態；最新 Vercel Preview checks 已通過，但未建立 production deployment。
